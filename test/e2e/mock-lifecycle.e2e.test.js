/**
 * E2E — Full mock lifecycle (Sprint 0.5 acceptance #6).
 *
 * Builds a ctx with FakeDb + mock stacks/xreserve/yellowcard adapters and
 * drives one disbursement from create to SETTLED, asserting at every hop that
 * the row's status advanced and the four persisted id columns are populated
 * (G-05). A second case asserts the negative path: a failed preflight stops at
 * MANUAL_REVIEW and never burns (G-01).
 *
 * No network, no Postgres, no credentials: the default suite stays green.
 *
 * Known fidelity gap (reported in SPRINT_0_5_REPORT.md, not hidden): a mock
 * xreserve returns "release confirmed" without external assurance (GAP-08), so
 * this proves orchestration completes, not that the release is externally true.
 */
...
function buildHarness() {
  ...
  // xReserve guards require status 'confirmed' (the mock default is 'complete').
  ...
  adapters.xreserve.getReleaseStatus = async (id) => {
    adapters.xreserve.calls.getReleaseStatus.push(id);
    return { release_id: id, status: 'confirmed' };
  };
  ...
  const refs = {
    ...
    'xreserve|release_id': { external_system: 'xreserve', identifier_type: 'release_id', identifier_value: 'rel-mock-1', metadata: {} },
 ...
 */
async function assertFullLifecycle({ db, adapters }) {
  ...
  const path = [
    ...
    [S.DESTINATION_RELEASE_SUBMITTED, 'release_id'],
    [S.DESTINATION_RELEASE_CONFIRMED, null],
    [S.YELLOWCARD_PAYOUT_SUBMITTED, 'payout_id'],
    [S.YELLOWCARD_PAYOUT_CONFIRMED, null],
    [S.SETTLED, null],
  ];

  for (const [nextState, idColumn] of path) {
  ...
  assert.equal(settled.release_id, 'rel-mock-1', 'release id persisted');
  ...
  // Evidence chain holds the burn tx hash + API responses.
...
test('E2E: full mock lifecycle drives one payout to SETTLED', async () => {
  const { db, adapters } = buildHarness();
  const settled = await assertFullLifecycle({ db, adapters });
  assert.equal(settled.status, S.SETTLED);
});
